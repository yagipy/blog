...
Althea IRによって所有権付き参照カウントのアルゴリズムを独立させ、かつCodegenの実装を簡単にすることができました。
<div>
...
参照カウントと所有権は、それぞれ単体でもメモリ管理手法として使用されていますが、Altheaでは組み合わせて使用しています。
...
ちなみにポインタが指すヒープ領域のデータはコピーしません。shallow copyと似ていますがムーブ元の変数が使えなくなるという点で異なるため、ムーブと呼ばれているようです。
このようにして、ムーブ元の変数(ここでは`foo`)は所有権を失い使用することができないためエラーとなります。
<div><br /></div>
...
- `bar`定義時`foo`の参照を渡す
  - 参照と呼ばれる方法です。オブジェクトの所有者は`foo`のままで、`bar`は`foo`の参照を保持します。`bar`は所有者ではないため、スコープを抜けてもオブジェクトは解放されません。(`foo`と`bar`のスコープが一緒なのでここでは少し紛らわしいですが...)
```rust
fn main() {
  let foo = String::from("foo");
  let bar = &foo;
...
- `foo`定義時`Arc`を使用する
  - マルチスレッドでデータを共有使用する場合は`Rc`ではなく`Arc`を使用します
  - `Rc`や`Arc`の参照先の値を変更したい時は`Cell`、`RefCell`、`Mutex`、`RwLock`を使用することができます
...
このように参照カウントの増減処理が不要であるにも関わらず発生してしまうケースは多くあります。

### 組み合わせるとどうなるか(所有権付き参照カウントの詳細)
所有権と参照カウントを組み合わせることで、先ほど提示したそれぞれの欠点を打ち消し合うことができます。
...
参照カウントの課題である「無駄なカウント管理処理が発生しやすい」は、ヒープ領域割り当てごとに所有権を割り当て、その後の利用を基本的に借用とすることで解消することができると考えています。
...
## 3つの課題がどのように解決されたか
ここからは、所有権付き参照カウントを用いることでプログラミング時に発生する「性能が安定しない」「手動メモリ管理による脆弱性」「学習コストの増大」という3つの課題を解決することができることを書いていきます。

### ①性能が安定しない
現在主流の言語で採用されているGCの多くはStop The Worldという処理が完全に停止する期間があるため、安定した性能を実現することは難しいことが多いです。具体例として[DiscordのRead Statesサービス](https://discord.com/blog/why-discord-is-switching-from-go-to-rust)や、[DMMのCassandraサーバー](https://inside.dmm.com/articles/cassandra-fullgc)があります。
<div><br /></div>
...
以下は出力されるLLVM IRの一部です。リクエストハンドラであるhandle内で構造体のメモリを解放(`@handle`関数の7行目)しているため、基本的に1リクエストごとにメモリが解放されます。
```llvm
define i8* @handle() {
entry:
  %malloccall = tail call i8* bitcast (i8* (i64)* @malloc to i8* (i32)*)(i32 ptrtoint ({ i32, i32 }* getelementptr ({ i32, i32 }, { i32, i32 }* null, i32 1) to i32))
...
### ②手動メモリ管理による脆弱性
手動でメモリ管理を行う言語は、Use-after-freeやメモリリーク等の問題が発生する可能性があり場合によっては重大な脆弱性につながります。[Chromiumでは、深刻なセキュリティバグの約70％がメモリ安全でないことが起因となっています。](https://www.chromium.org/Home/chromium-security/memory-safety)
<div><br /></div>
...
### ③学習コストの増大
課題の①と②を解決するために所有権を使う言語がありますが、所有権を使う言語は学習コストが高いと言われています。
[2016年に行われたRustの調査](https://blog.rust-lang.org/2016/06/30/State-of-Rust-Survey-2016.html)ではRustの課題について1900人以上から回答があり、その中の4人に1人が学習曲線についてコメントをしています。
下記はそのコメントの一部です。
...
### HTTPサーバー
Stop The Worldによって性能要件を満たすことが難しいサーバーや、大規模にメモリを使用するサーバーにより効果的だと考えています。
サポートが進んでおり、現在2ファイルでHTTPサーバーを立てることが可能になっています。今後は様々なHTTPメソッドのハンドリングやクエリパラメータの取得をサポートする予定です。
