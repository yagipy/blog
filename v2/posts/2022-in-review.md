...
*golangci-lint*については、コントリビュート後にGitHubのOrganizationに招待されメンバーとなることができました。
<div>
...
*althea*の特徴は所有権付き参照カウントというメモリ管理手法を実装していることです。このメモリ管理手法によって既存言語の課題を解決しています。他には基本的な言語機能に加えてHTTPサーバーを簡単に起動できたりします。実装言語はRustで、LLVMをバックエンドにしています。
...
その「自作で似た分野のOSSを作成する」という目標は、*althea*を作成することによって達成できたので良かったです。
